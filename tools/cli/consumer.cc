...
int RunConsumer(int argc, char* argv[]) {
  ...
  grpc::Status assignment_status = coordinator_stub->PollAssignment(
    &assignment_context, assignment_request, &assignment_response);
  ...
  do {
    ...
    for (const auto& batch : fetch_response.batches()) {
      for (const auto& record : batch.records()) {
        std::cout << "[" << record.timestamp_ms() << "] " 
                  << "key=" << record.key() 
                  << " value=" << record.value().substr(0, 50) 
    ...
    if (total_messages > 0) {
      ...
      grpc::Status commit_status = coordinator_stub->CommitOffset(
