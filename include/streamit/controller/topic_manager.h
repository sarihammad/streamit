...
public:
  ...
  // Create a new topic
  [[nodiscard]] Result<void> CreateTopic(const std::string& name, 
                                        int32_t partitions, 
  ...
  [[nodiscard]] Result<void> UpdatePartitionLeader(const std::string& topic, 
                                                  int32_t partition, 
                                                  int32_t leader) noexcept;
  
  // Update partition high water mark
  [[nodiscard]] Result<void> UpdatePartitionHighWaterMark(const std::string& topic, 
                                                         int32_t partition, 
                                                         int64_t high_watermark) noexcept;
  
  // Get partition information
  [[nodiscard]] Result<PartitionInfo> GetPartitionInfo(const std::string& topic, 
...
private:
  ...
  [[nodiscard]] std::vector<PartitionInfo> GeneratePartitionAssignments(
