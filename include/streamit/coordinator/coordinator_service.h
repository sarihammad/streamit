...
public:
  ...
  grpc::Status CommitOffset(grpc::ServerContext* context, const streamit::v1::CommitOffsetRequest* request,
                            streamit::v1::CommitOffsetResponse* response) override;

  // PollAssignment RPC implementation
  grpc::Status PollAssignment(grpc::ServerContext* context, const streamit::v1::PollAssignmentRequest* request,
...
private:
  ...
  [[nodiscard]] grpc::Status ValidateCommitOffsetRequest(const streamit::v1::CommitOffsetRequest* request) const;

  // Helper to validate poll assignment request
  [[nodiscard]] grpc::Status ValidatePollAssignmentRequest(const streamit::v1::PollAssignmentRequest* request) const;
};
...
public:
  // Constructor
  CoordinatorServer(const std::string& host, uint16_t port, std::shared_ptr<ConsumerGroupManager> group_manager);
