...
struct ConsumerMember {
  ...
  ConsumerMember(std::string member_id, std::vector<std::string> topics)
      : member_id(std::move(member_id)), topics(std::move(topics)), last_heartbeat(std::chrono::steady_clock::now()),
        active(true) {
  }
};
...
public:
  ...
  [[nodiscard]] Result<void> JoinGroup(const std::string& group_id, const std::string& member_id,
                                       const std::vector<std::string>& topics) noexcept;

  // Leave a consumer group
  [[nodiscard]] Result<void> LeaveGroup(const std::string& group_id, const std::string& member_id) noexcept;

  // Heartbeat from a consumer
  [[nodiscard]] Result<void> Heartbeat(const std::string& group_id, const std::string& member_id) noexcept;

  // Get partition assignments for a consumer
  [[nodiscard]] Result<std::vector<PartitionAssignment>> GetAssignments(const std::string& group_id,
                                                                        const std::string& member_id) const noexcept;

  // Commit offset for a partition
  [[nodiscard]] Result<void> CommitOffset(const std::string& group_id, const std::string& topic, int32_t partition,
                                          int64_t offset) noexcept;

  // Get committed offset for a partition
  [[nodiscard]] Result<int64_t> GetCommittedOffset(const std::string& group_id, const std::string& topic,
...
private:
  ...
  [[nodiscard]] std::unordered_map<std::string, std::vector<PartitionAssignment>> AssignPartitions(
  ...
  [[nodiscard]] std::unordered_set<std::string> GetGroupTopics(const ConsumerGroup& group) const noexcept;
};
