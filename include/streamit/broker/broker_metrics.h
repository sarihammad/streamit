...
public:
  ...
  // Produce metrics
  void RecordProduceLatency(const std::string& ack, const std::string& topic, 
                           int32_t partition, double latency_ms) noexcept;
  void RecordProduceBytes(const std::string& topic, int32_t partition, 
                         int64_t bytes) noexcept;
  void RecordProduceRecords(const std::string& topic, int32_t partition, 
                           int64_t records) noexcept;
  
  // Fetch metrics
  void RecordFetchLatency(const std::string& topic, int32_t partition, 
                         double latency_ms) noexcept;
  void RecordFetchBytes(const std::string& topic, int32_t partition, 
                       int64_t bytes) noexcept;
  ...
  void SetHighWaterMark(const std::string& topic, int32_t partition, 
                       int64_t offset) noexcept;
  
  // Replication lag metrics (for future use)
  void SetReplicationLag(const std::string& topic, int32_t partition, 
