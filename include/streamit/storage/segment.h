...
public:
  ...
  Segment(std::filesystem::path log_path, std::filesystem::path index_path, 
          int64_t base_offset, size_t max_size_bytes, FlushPolicy flush_policy = FlushPolicy::OnRoll);
  
  // Open an existing segment
  static Result<std::unique_ptr<Segment>> Open(std::filesystem::path log_path, 
  ...
  [[nodiscard]] Result<std::vector<RecordBatch>> Read(int64_t from_offset, 
...
private:
  ...
  Segment(std::filesystem::path log_path, std::filesystem::path index_path, 
          int64_t base_offset, size_t max_size_bytes, int64_t end_offset);
  ...
  [[nodiscard]] Result<std::vector<std::byte>> ReadLogData(int64_t position, 
