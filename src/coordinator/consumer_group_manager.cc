...
ConsumerGroupManager::ConsumerGroupManager(int32_t heartbeat_interval_ms, 
                                          int32_t session_timeout_ms)
  : heartbeat_interval_ms_(heartbeat_interval_ms)
  , session_timeout_ms_(session_timeout_ms) {}

Result<void> ConsumerGroupManager::JoinGroup(const std::string& group_id, 
...
Result<void> ConsumerGroupManager::LeaveGroup(const std::string& group_id, 
...
Result<void> ConsumerGroupManager::Heartbeat(const std::string& group_id, 
                                            const std::string& member_id) noexcept {
  std::lock_guard<std::mutex> lock(mutex_);
...
Result<std::vector<PartitionAssignment>> ConsumerGroupManager::GetAssignments(
  ...
  auto group_it = groups_.find(group_id);
  if (group_it == groups_.end()) {
    return Error<std::vector<PartitionAssignment>>(absl::StatusCode::kNotFound, 
...
Result<void> ConsumerGroupManager::CommitOffset(const std::string& group_id,
...
Result<int64_t> ConsumerGroupManager::GetCommittedOffset(const std::string& group_id,
                                                        const std::string& topic,
...
std::unordered_map<std::string, std::vector<PartitionAssignment>> 
ConsumerGroupManager::AssignPartitions(const ConsumerGroup& group) const noexcept {
...
std::unordered_set<std::string> ConsumerGroupManager::GetGroupTopics(
