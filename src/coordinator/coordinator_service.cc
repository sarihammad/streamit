...
grpc::Status CoordinatorServiceImpl::CommitOffset(grpc::ServerContext* context,
  ...
  // Commit offset
  auto commit_result =
      group_manager_->CommitOffset(request->group(), request->topic(), request->partition(), request->offset());
...
grpc::Status CoordinatorServiceImpl::PollAssignment(grpc::ServerContext* context,
  ...
  // Join group
  auto join_result =
      group_manager_->JoinGroup(request->group(), request->member_id(),
...
CoordinatorServer::CoordinatorServer(const std::string& host, uint16_t port,
    ...
    : host_(host), port_(port), group_manager_(std::move(group_manager)), running_(false) {
}
