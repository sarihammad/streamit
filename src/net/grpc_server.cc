...
GrpcServerBuilder::GrpcServerBuilder(const std::string& host, uint16_t port)
  : host_(host)
  , port_(port)
  , max_send_message_size_(4 * 1024 * 1024)    // 4MB
  , max_receive_message_size_(4 * 1024 * 1024)  // 4MB
  , keepalive_time_ms_(30000)                   // 30 seconds
  , keepalive_timeout_ms_(5000)                 // 5 seconds
  , keepalive_permit_without_calls_(true)
  , max_connection_idle_ms_(300000)             // 5 minutes
...
GrpcServerBuilder& GrpcServerBuilder::SetMaxMessageSize(size_t max_send_size, 
...
GrpcServerBuilder& GrpcServerBuilder::SetKeepaliveOptions(int64_t keepalive_time_ms,
...
std::unique_ptr<grpc::Server> GrpcServerBuilder::Build() {
  ...
  grpc::ServerBuilder::Option option3(GRPC_ARG_KEEPALIVE_PERMIT_WITHOUT_CALLS, 
  ...
  grpc::ServerBuilder::Option option6(GRPC_ARG_MAX_CONNECTION_AGE_GRACE_MS, 
  ...
  // Set thread pool
  std::unique_ptr<grpc::ThreadPoolInterface> thread_pool = 
    std::make_unique<grpc::ThreadPool>(num_threads_);
  builder.SetThreadPool(thread_pool.release());
  
  return builder.BuildAndStart();
}

GrpcServer::GrpcServer(std::unique_ptr<grpc::Server> server)
