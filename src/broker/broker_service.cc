...
BrokerServiceImpl::BrokerServiceImpl(std::shared_ptr<storage::LogDir> log_dir,
  ...
  , idempotency_table_(std::move(idempotency_table))
  , metrics_(std::make_unique<BrokerMetrics>()) {}

grpc::Status BrokerServiceImpl::Produce(grpc::ServerContext* context,
  ...
  // Log request
  streamit::common::StructuredLogger::Info(trace_id, 
    "Produce request: topic={}, partition={}, records={}, ack={}", 
  ...
  auto hwm_result = log_dir_->SetHighWaterMark(request->topic(), request->partition(), 
                                               base_offset + records.size());
  if (!hwm_result.ok()) {
  ...
  // Log success
  streamit::common::StructuredLogger::Info(trace_id, 
    "Produce completed: base_offset={}, latency_ms={}", base_offset, latency_ms);
  
  return grpc::Status::OK;
}

grpc::Status BrokerServiceImpl::Fetch(grpc::ServerContext* context,
  ...
  // Log request
  streamit::common::StructuredLogger::Info(trace_id, 
    "Fetch request: topic={}, partition={}, offset={}, max_bytes={}", 
    request->topic(), request->partition(), request->offset(), request->max_bytes());
  ...
  for (const auto& segment : segments) {
    if (request->offset() >= segment->BaseOffset() && 
        request->offset() < segment->EndOffset()) {
  ...
  // Log success
  streamit::common::StructuredLogger::Info(trace_id, 
    "Fetch completed: batches={}, bytes={}, latency_ms={}", 
...
void BrokerServiceImpl::ConvertRecords(
...
BrokerServer::BrokerServer(const std::string& host, uint16_t port,
