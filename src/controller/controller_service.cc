...
grpc::Status ControllerServiceImpl::CreateTopic(grpc::ServerContext* context,
  ...
  // Create topic
  auto create_result = topic_manager_->CreateTopic(
    request->topic(), request->partitions(), request->replication_factor());
...
grpc::Status ControllerServiceImpl::FindLeader(grpc::ServerContext* context,
  ...
  // Find the partition
  auto partition_it = std::find_if(topic_info.partition_infos.begin(), 
                                  topic_info.partition_infos.end(),
                                  [&](const auto& partition) {
                                    return partition.partition == request->partition();
                                  });
...
grpc::Status ControllerServiceImpl::ValidateCreateTopicRequest(
    const streamit::v1::CreateTopicRequest* request) const {
...
grpc::Status ControllerServiceImpl::ValidateFindLeaderRequest(
    const streamit::v1::FindLeaderRequest* request) const {
  if (request->topic().empty()) {
    return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "Topic name cannot be empty");
  }
...
ControllerServer::ControllerServer(const std::string& host, uint16_t port,
