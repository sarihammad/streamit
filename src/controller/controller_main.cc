...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    spdlog::info("Starting StreamIt controller {} on {}:{}", 
    ...
    for (const auto& topic_config : topics) {
      auto create_result = topic_manager->CreateTopic(
        topic_config.name, topic_config.partitions, topic_config.replication_factor);
      if (create_result.ok()) {
        spdlog::info("Loaded topic: {} with {} partitions", 
                    topic_config.name, topic_config.partitions);
      } else {
        spdlog::warn("Failed to load topic {}: {}", 
                    topic_config.name, create_result.status().message());
      }
    }
    
    // Create and start server
    g_server = std::make_unique<streamit::controller::ControllerServer>(
      config.host, config.port, topic_manager);
