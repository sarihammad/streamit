...
Index::Index(std::filesystem::path index_path) : index_path_(std::move(index_path)), index_fd_(-1) {
...
Result<std::unique_ptr<Index>> Index::Open(std::filesystem::path index_path) {
  ...
  if (!std::filesystem::exists(index_path)) {
    return Error<std::unique_ptr<Index>>(absl::StatusCode::kNotFound, "Index file not found: " + index_path.string());
  }
  ...
  if (!load_result.ok()) {
    return Error<std::unique_ptr<Index>>(load_result.status().code(), load_result.status().message());
  }

  return Ok(std::move(index));
}

Result<void> Index::AddEntry(int64_t relative_offset, int64_t file_position, int32_t batch_size) noexcept {
...
Result<std::vector<IndexEntry>> Index::GetEntries(int64_t from_offset, int64_t to_offset) const noexcept {
