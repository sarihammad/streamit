...
Result<RecordBatch> Serializer::DeserializeBatch(std::span<const std::byte> data) noexcept {
  ...
  if (!base_offset_result.ok()) return Error<RecordBatch>(base_offset_result.status());
  
  auto timestamp_result = ReadInt64(remaining_data);
  if (!timestamp_result.ok()) return Error<RecordBatch>(timestamp_result.status());
  
  auto record_count_result = ReadInt32(remaining_data);
  if (!record_count_result.ok()) return Error<RecordBatch>(record_count_result.status());
  ...
  if (!crc32_result.ok()) return Error<RecordBatch>(crc32_result.status());
...
Result<Record> Serializer::DeserializeRecord(std::span<const std::byte> data) noexcept {
  ...
  if (!key_len_result.ok()) return Error<Record>(key_len_result.status());
  ...
  if (!value_len_result.ok()) return Error<Record>(value_len_result.status());
  ...
  auto timestamp_result = ReadInt64(remaining_data);
  if (!timestamp_result.ok()) return Error<Record>(timestamp_result.status());
