...
void StructuredLogger::Initialize(const std::string& level) {
  ...
  auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>("logs/streamit.json", 1024 * 1024 * 10,
                                                                          5); // 10MB, 5 files
  file_sink->set_pattern(R"({"timestamp":"%Y-%m-%dT%H:%M:%S.%eZ","level":"%l","thread":"%t","message":"%v"})");

  // Create logger with both sinks
  auto logger = std::make_shared<spdlog::logger>("streamit", spdlog::sinks_init_list{console_sink, file_sink});
